...
Class AdminImageService extends CI_Model
{
	...
	public function generateThumbsStoreData(){
		if (is_null($this->completePercent())){
			$result				= new stdClass();
			$result->name		= AdminImageService::CONVERT_PROCESS;
			$result->percent	= 0;
			$this->db->insert("process", $result);

			// Clear out the database
			$this->db->query("DELETE FROM thumb_row");
			...
			$this->db->delete("process", array('name' => AdminImageService::CONVERT_PROCESS));
		}
	}
		...
		for ($r = 0; $r < count($rows); $r++) {
			...
			for ($w = 0; $w < count($pageWidths); $w++){
				...
				for ($i = 0; $i < count($currArr); $i++) {
					...
					$process->name		= AdminImageService::CONVERT_PROCESS;
